...
public interface ApprovalJpaRepository extends JpaRepository<Approval, ApprovalId> {
    ...
    @Query("SELECT a FROM Approval a WHERE a.eventChangeItemId.id IN :ids ORDER BY a.decidedAt ASC")
    List<Approval> findByEventChangeItemIdIn(@Param("ids") Collection<UUID> ids);
}
